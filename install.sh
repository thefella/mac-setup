...
REMOTE_BREWFILE="https://raw.githubusercontent.com/thefella/mac-setup/refs/heads/main/basic.Brewfile"
#MAIN_BREWFILE="https://raw.githubusercontent.com/thefella/mac-setup/refs/heads/main/main.Brewfile"


# General Config
PARAMS=$* # User-specified parameters
CURRENT_DIR=$(cd "$(dirname ${BASH_SOURCE[0]})" && pwd)
PROMPT_TIMEOUT=12 # When user is prompted for input, skip after x seconds
...
show_intro () {
  echo -e "\n${QUESTION_COLOR}🍺 thefella/Brewfile - Installation Script${RESET}"
...
run_macos_hardening () {
  echo -en "\n🍺  ${HEADING_COLOR}Hardening MacOS${RESET}\n"
  /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/thefella/mac-setup/refs/heads/main/apple.sh)"
}


# Confirm user is happy to proceed with MacOS Hardening
echo -e "\n${QUESTION_COLOR}Proceed with hardening MacOS? (y/N)${RESET}"
read -t $PROMPT_TIMEOUT -n 1 -r
if [[ $REPLY =~ ^[Yy]$ ]]; then
  run_macos_hardening
fi
